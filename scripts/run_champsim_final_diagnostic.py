#!/usr/bin/env python3
"""Run opt-in WAM pipeline diagnostics on the frozen native ChampSim traces."""
...
DEFAULT_RAW = Path("/tmp/wam_final_diagnostic_raw")
...
def run_one(job: dict[str, object]) -> dict[str, object]:
    ...
    env = os.environ.copy()
    env["WAM_DIAGNOSTIC_EVENT_PATH"] = str(event_path)
    env["WAM_DIAGNOSTIC_PREDICTION_PATH"] = str(prediction_path)
    started = time.time()
    completed = subprocess.run(command, cwd=str(job["champsim_root"]), env=env, text=True, capture_output=True, check=False)
...
def main() -> None:
    ...
    parser.add_argument("--executable", type=Path, default=ROOT / "external/champsim/bin/champsim_wam_h16")
    ...
    if not args.executable.resolve().exists():
        raise SystemExit(f"diagnostic executable not found: {args.executable}")
    ...
    for row in rows:
        ...
        if args.resume and (run_output / "run.json").exists():
            try:
                ...
                pass
        jobs.append({"trace_name": row["trace_name"], "trace": trace_path, "output": run_output, "raw": run_raw, "champsim_root": champsim_root, "executable": args.executable.resolve(), "warmup": args.warmup, "simulation": args.simulation})
    results = []
    ...
    with (output / "diagnostic_runs.csv").open("w", newline="", encoding="utf-8") as handle:
