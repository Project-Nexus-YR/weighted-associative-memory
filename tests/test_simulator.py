...
def test_baseline_and_prefetch_metrics_are_deterministic():
    ...
    config = SimulatorConfig(
        ...
        hierarchy=HierarchyConfig(cache_line_size=1, l1_latency=1, l2_latency=2, l3_latency=3, dram_latency=2),
    )
    ...
    # With an outstanding-request model, a one-line-ahead prefetch can still
    # arrive too late; its raw demand work should nevertheless be reduced.
    assert predictive.metrics.raw_memory_cycles < baseline.metrics.raw_memory_cycles


def test_contextual_traces_reward_deeper_predictor():
    trace = contextual(30)
    predictor = WeightedTriePredictor(context_depth=2).fit(trace[:100])
    result = simulate(trace[100:], predictor, SimulatorConfig(hierarchy=HierarchyConfig(cache_line_size=1)))
...
def test_random_workload_can_issue_no_or_few_prefetches_with_threshold():
    ...
    result = simulate(trace[40:], predictor, SimulatorConfig(hierarchy=HierarchyConfig(cache_line_size=1)))
